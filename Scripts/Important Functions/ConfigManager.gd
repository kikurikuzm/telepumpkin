...
const ORIGINAL_CONFIG_FILE := "res://default_config.ini"
...
func initializeConfig() -> void:
	...
	var err := config_file.load(ORIGINAL_CONFIG_FILE)
	if err != OK:
...
func readControlValues(parent_node: Node) -> void:
	for child in parent_node.get_children():
		...
		if child is CheckBox:
			...
			if true_value and child.button_pressed: option_value = true_value
			elif false_value and not child.button_pressed: option_value = false_value
